...
#Load banks lists, annotate them to geneSymbol
 load('genesets/gsMain_PGC_2021.RData')
  ...
  hs <- org.Hs.eg.db
#notice how annotationdbi:: select is used, otherwise the function gets masked
...
for (gtype in c("all","dup","del")) # all",
{
 ...
 #remove NDD with Genome-wideBurdenNoImplicated_Both
 ...
 #Create a matrix to store all meta-analyses 
  ...
  for (i in unique(geneSetMat$geneset)) # 1:ceiling((dim(genes_matrix)[2]/increment)))
  {
   ...
   #Loop over studies
    ...
    {
      results <- try(summary(clm(as.factor(pheno) ~ C1 + C2 + C3 + C4 + C5 + LRR_SD  + cnv_count + cnv_avg  + genes_subset_count_study, data=cnv_fam_subset))$coefficients ,silent=TRUE)
     ...
     write.table(resmat2,file=paste("gene_setR_jun14_2022/",gtype,"_",i,"_allstudies_nddremoved.txt",sep=""),row.names=F)
     res <- rma(yi=resmat2$beta,sei=resmat2$se,slab=resmat2$study,method="FE")
     meta_results[which(meta_results$geneSet ==i),2:5] <- c(res$beta,res$se,res$zval,res$pval) 
    }

    write.table(meta_results,file=paste("gene_setR_jun14_2022/",gtype,"_allmetas_nddremoved.txt",sep=""),row.names=F)
