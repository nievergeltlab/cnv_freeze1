...
#Results will be stored in this style of matrix
 ...
 for (feature in c("one"))#"both","one"))
 {
 #for main analysis; 26 fails, 47 fails, 50 fails - -c(26,47,50)
 #For one: 
 #main analysis, 0% overlap: c(1:9,11:25,27:46,48:49,51:nrow(cnvparms)) 10 failes,26
 
  #  1:11,
 for (i in c(1:nrow(cnvparms)))
 {
  print(i)
  #store results
   resmat2 <- resmat
  ...
  for (cnvtype in c("cnv_count","cnv_avg","cnv_span"))
  {
   #And over study
   ...
   outfile=paste("neurocnv_100overlap_",i,"_",feature,"_",cnvtype,sep="")
   
   #Only do analyses if there are values to test...
   if(! all(is.na(resmat2$beta)))
   {
   write.table(resmat2,file=paste("outputs_psychiatric_cnvlevel/","bystudy_",outfile,'.txt',sep=''),row.names=F)
   meta_res <- rma(yi=resmat2$beta_scale,sei=resmat2$se_scale,slab=resmat2$study,method="FE")
   capture.output(meta_res,paste("outputs_psychiatric_cnvlevel/","meta_",outfile,'.txt',sep=''))
   ...
   write.table(meta_out,paste("outputs_psychiatric_cnvlevel/","meta2_",outfile,'.txt',sep=''),row.names=F)

  pdf(paste("outputs_psychiatric_cnvlevel/","meta_",outfile,'.pdf',sep=''),7,7)
   plot(meta_res)
  dev.off()
  pdf(paste("outputs_psychiatric_cnvlevel/","forest_",outfile,'.pdf',sep=''),7,7)
   forest(meta_res)
  dev.off()
  }
 ...
 for files in $(ls outputs_psychiatric_cnvlevel | grep meta2 | grep 100overlap | grep count)
 do
  output=$( tail -n1 outputs_psychiatric_cnvlevel/$files)
  echo $files $output >> cnv_burden_psychiatric_cnvlevel_100overlap_dec17_2021.txt
 done
